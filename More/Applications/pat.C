...
void usage ()
{
  cout << "pat - Pulsar::Archive timing \n"
    ...
    "\n"
    "Channelized DFT template matching options: \n"
    "  -X               Enable 2D template matching with CDFT \n"
    "  -y               Factor of frequency scrunch before CDFT \n"
    "  -w               Enable simultaneous fit for DM \n"
    "\n"
    "Output options:\n"
...
int main (int argc, char** argv) try
{
  ...
  // The 2D CDFT method and related flags
  Pulsar::tDTemplateMatching_CDFT* td_cdft = 0;
  unsigned fsfac = 1;
  unsigned dmsign = 0;
...
#define PLOT_ARGS
  ...
  const char* args = "a:A:bcC:Dde:E:f:Fg:G:hij:J:K:m:M:n:pPqRrS:s:TuvVwXx:y:z:" PLOT_ARGS;
  ...
  while ((gotc = getopt(argc, argv, args)) != -1)
  {
    ...
    case 'X':
      arrival = td_cdft = new Pulsar::tDTemplateMatching_CDFT;
      break;
  ...
  if (td_cdft)
    {
      cerr << "pat: using CDFT 2D template matching" << endl;

      stdarch = Archive::load(std);

      stdarch->tscrunch();

      td_cdft->set_fscrunch_factor (fsfac);

      td_cdft->set_dmfit (dmsign);
  ...
  if (!std.empty() && !std_multiple && !gaussian && !td_cdft) try
  {
    stdarch = Archive::load(std);
